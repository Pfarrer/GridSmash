...
func test_empty_energy_grid():
	...
	assert_eq(energy_grid.energy_generation_max(), 0)
	assert_eq(energy_grid.energy_consumption_max(), 0)
	assert_eq(energy_grid.energy_capacity_max(), 0)
...
func test_grid_node_only_energy_grid():
	...
	assert_eq(energy_grid.energy_generation_max(), 2 * GN_ENERGY_GENERATION)
	assert_signal_emitted_with_parameters(energy_grid, "energy_generation_max_changed", [2 * GN_ENERGY_GENERATION])
	
	var connection2 = GridConnection.new(grid_node_structure1, grid_node_structure3)
	energy_grid.add_grid_connection(connection2)
	assert_eq(energy_grid.is_empty(), false)
	assert_eq(energy_grid.energy_generation_max(), 3 * GN_ENERGY_GENERATION)
	assert_signal_emitted_with_parameters(energy_grid, "energy_generation_max_changed", [3 * GN_ENERGY_GENERATION])
	
	var connection3 = GridConnection.new(grid_node_structure3, grid_node_structure2)
	energy_grid.add_grid_connection(connection3)
	assert_eq(energy_grid.is_empty(), false)
	assert_eq(energy_grid.energy_generation_max(), 3 * GN_ENERGY_GENERATION)
	assert_signal_emit_count(energy_grid, "energy_generation_max_changed", 2)
	...
	assert_eq(energy_grid.is_empty(), true)
	assert_eq(energy_grid.energy_generation_max(), 0)
	assert_signal_emitted_with_parameters(energy_grid, "energy_generation_max_changed", [0])
	
	assert_signal_emit_count(energy_grid, "energy_generation_max_changed", 4)
...
func test_merge_with_grid():
	...
	var expected_generation = energy_grid.energy_generation_max() + other_grid.energy_generation_max()
	var expected_consumption = energy_grid.energy_consumption_max() + other_grid.energy_consumption_max() 
	var expected_capacity = energy_grid.energy_capacity_max() + other_grid.energy_capacity_max() 
	energy_grid.merge_with_grid(other_grid)

	assert_eq(energy_grid.energy_generation_max(), expected_generation)
	assert_eq(energy_grid.energy_consumption_max(), expected_consumption)
	assert_eq(energy_grid.energy_capacity_max(), expected_capacity)
